...
impl Engine {
  ...
  pub fn new(port: u16) -> Engine {
    ...
    planner.add_system(io::network_adapter::System::new(port),
                       "network::io",
                       NETWORK_IO_PRIORITY);
    ...
    planner.add_system(player::connection::System::new(),
                       "player::connection",
                       PLAYER_CONNECTION_PRIORITY);
    planner.add_system(player::snapshot::System::new(),
                       "player::snapshot",
                       PLAYER_SNAPSHOT_PRIORITY);
    planner.add_system(player::input::System::new(),
                       "player::input",
                       PLAYER_INPUT_PRIORITY);
