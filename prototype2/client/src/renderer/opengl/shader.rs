...
pub mod constants {
  ...
  pub fn cube_shader() -> Shader {
    let glsl_150v: Vec<u8> =
      File::open("../assets/shaders/cube_150.glslv").unwrap().bytes().map(|x| x.unwrap()).collect();
    let glsl_150f: Vec<u8> =
      File::open("../assets/shaders/cube_150.glslf").unwrap().bytes().map(|x| x.unwrap()).collect();
